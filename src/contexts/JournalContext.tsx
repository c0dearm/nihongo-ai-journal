...
export const JournalProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    localStorage.setItem(LocalStorageKeys.journalEntries, JSON.stringify(entries));
  }, [entries]);
  ...
  const addEntry = useCallback(async (text: string) => {
    ...
    if (isApiKeySet) {
      try {
        ...
        setEntries(prev => prev.map(e => e.id === newEntry.id ? { ...e, feedback, isLoading: false } : e));
      } catch (error) {
  ...
  const updateEntry = useCallback((id: string, text: string) => {
      setEntries(prev => prev.map(e => e.id === id ? { ...e, originalText: text, feedback: null } : e));
  }, []);
  ...
  const refreshFeedback = useCallback(async (id: string) => {
      ...
      try {
          const feedback = await getJournalFeedback(entry.originalText, entry.jlptLevel);
          setEntries(prev => prev.map(e => e.id === id ? { ...e, feedback, isLoading: false } : e));
      } catch (error) {
  ...
  return (
    <JournalContext.Provider value={{ entries, addEntry, deleteEntry, updateEntry, exportJournal, refreshFeedback }}>
      {children}
