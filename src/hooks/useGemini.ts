...
import { Feedback, JLPTLevel, JournalEntry, LocalStorageKeys } from "../types";
...
export const useGemini = () => {
  const [ai, setAi] = useState<GoogleGenAI | null>(() => {
    const savedApiKey = localStorage.getItem(LocalStorageKeys.geminiApiKey);
    return savedApiKey ? new GoogleGenAI({ apiKey: savedApiKey }) : null;
  });
...
`;
  ...
  const sendChatMessage = useCallback(
    async (message: string, onChunk: (chunk: string) => void) => {
      ...
      try {
        const stream = await chat.current.sendMessageStream({
          message: message,
        });
