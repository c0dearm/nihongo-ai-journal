...
import { BookOpenIcon, GeminiIcon, SettingsIcon, SparklesIcon, UserIcon } from '../ui/Icons';
...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onViewChange }) => {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchTerm.trim()) {
      window.open(`https://jisho.org/search/${encodeURIComponent(searchTerm)}`, '_blank');
      setSearchTerm('');
    }
  };
      ...
      <div className="p-4 border-t border-gray-200 dark:border-gray-800">
        <div className="flex items-center space-x-3 px-3 py-2">
           ...
           <div className="text-sm">
             <p className="font-medium text-gray-700 dark:text-gray-200">Learner</p>
           </div>
        </div>
      </div>
    </aside>
